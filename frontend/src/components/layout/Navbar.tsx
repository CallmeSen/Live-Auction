...
export default function Navbar() {
  ...
  const visibleItems = navItems.filter((item) => !item.roles || (session && item.roles.includes(session.role)));
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--color-border)] bg-[var(--color-bg)]/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-6">
        <Link to={roleHome} aria-label="Về trang chính"><Logo /></Link>
          ...
          {session ? (
            <>
              <div className="hidden text-right sm:block">
                <p className="max-w-32 truncate text-xs text-[var(--color-text)]">{session.email}</p>
                <p className="mt-0.5 text-[10px] text-[var(--color-primary)]">{session.role}</p>
              </div>
              <NotificationBell />
      ...
      {mobileOpen && (
        <div className="border-t border-[var(--color-border)] bg-[var(--color-bg)] px-6 py-5 lg:hidden">
          <nav className="flex flex-col gap-4">
            ...
            {session && <button onClick={() => void handleLogout()} disabled={loggingOut} className="border-t border-[var(--color-border)] pt-4 text-left text-sm text-[var(--color-danger)]">Đăng xuất</button>}
          </nav>
        </div>
      )}
    </header>
