...
export default function ProfilePage({ catalogApi }: ProfilePageProps) {
  ...
  const dirty = editing
    && (form.fullName !== savedForm.fullName || form.phone !== savedForm.phone);

  const initials = useMemo(() => (
    form.fullName || user?.email || 'A'
  )
    .split(/\s+/)
    .filter(Boolean)
    .slice(-2)
    .map((word) => word[0])
    .join('')
    .toUpperCase(), [form.fullName, user?.email]);
  ...
  const cancel = () => {
    if (dirty && !window.confirm('Thông tin chưa được lưu. Bạn có muốn bỏ các thay đổi không?')) {
      return;
    }
  ...
  const handleLogout = async () => {
    ...
    if (dirty && !window.confirm('Thông tin chưa được lưu. Bạn vẫn muốn đăng xuất?')) {
      return;
    }
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-12">
      ...
      <p className="mt-2 text-sm text-[var(--color-text-muted)]">
        Quản lý thông tin liên hệ và giao diện hiển thị của tài khoản.
      </p>

      <div className="mt-8 grid gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-7 text-center">
          <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full border border-[var(--color-primary)] font-display text-3xl text-[var(--color-primary)]">
            {initials}
          </div>
          ...
          <span className="mt-4 inline-flex rounded-full border border-[var(--color-success-border)] px-3 py-1 text-xs text-[var(--color-success)]">
            Đang hoạt động
          </span>
          <p className="mt-4 font-mono-tag text-xs uppercase tracking-[0.16em] text-[var(--color-primary)]">
            Thành viên
          </p>

          <div className="mt-7 grid grid-cols-2 overflow-hidden rounded-xl border border-[var(--color-border)]">
            <div className="p-4">
              <p className="font-display text-lg">Thành viên</p>
              <p className="mt-1 text-[10px] text-[var(--color-text-dim)]">Vai trò</p>
            </div>
            <div className="border-l border-[var(--color-border)] p-4">
              <p className="font-display text-lg">Hoạt động</p>
              <p className="mt-1 text-[10px] text-[var(--color-text-dim)]">Trạng thái</p>
            </div>
          </div>
          ...
          <Button
            type="button"
            variant="secondary"
            className="mt-6 w-full border-[var(--color-danger-border)] text-[var(--color-danger)]"
            disabled={loggingOut}
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-7">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="text-xs text-[var(--color-text-dim)]">
              {loadingProfile ? 'Đang tải...' : 'Dữ liệu tài khoản'}
            </span>
          </div>
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <Input
              label="Họ và tên"
              name="fullName"
              disabled={!editing || loadingProfile}
              value={form.fullName}
          ...
          {error && (
            <p className="mt-5 rounded-lg border border-[var(--color-danger-border)] p-3 text-sm text-[var(--color-danger)]">
              {error}
            </p>
          )}
          {message && (
            <p className="mt-5 rounded-lg border border-[var(--color-success-border)] p-3 text-sm text-[var(--color-success)]">
              {message}
            </p>
          )}
